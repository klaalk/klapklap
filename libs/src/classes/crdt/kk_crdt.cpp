...
vector<kk_identifier_ptr> kk_crdt::generate_position_between(vector<kk_identifier_ptr> position1, vector<kk_identifier_ptr> position2,
    ...
    strategy _strategy;
    int elev = pow(2,level);
    double _base = elev * this->base;
...
double kk_crdt::generate_identifier_between(int min, int max, strategy _strategy) {
    ...
    _rand = rand()%100;
    _rand=_rand/100;
     std::cout<<_rand<<std::endl;
     random_number=floor(_rand*(max- min) + min);
