...
class CRDT_Crdt {
    vector<list<CRDT_Char>> text;
    string siteid;
    int boundary;
    strategy _strategy;
    vector<strategy> strategy_cache;
    int base;
    vector<CRDT_identifier> slice(vector<CRDT_identifier> const &v, int i);
public:
...
//    int is_empty();
//    CRDT_pos find_end_position (CRDT_Char last_char,  list<CRDT_Char> last_line, int total_lines);
//    int find_insert_index_in_line(CRDT_Char _char, list<CRDT_Char> line);
