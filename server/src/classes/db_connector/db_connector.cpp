...
user_info* db_connector::db_getUserInfo(std::string username){
    ...
    sql::ResultSet *res=stmt->executeQuery("SELECT `ID`,`NAME`,`SURNAME`,`EMAIL`,`IMAGE`,`REGISTRATION_DATE`,`PASSWORD` FROM `USERS` WHERE `USERNAME`='"+username+"';");
    res->next();
...
int db_connector::db_insert_user(std::string username, std::string password, std::string email, std::string name,
    ...
    Poco::Crypto::Cipher* pCipher = factory.createCipher(Poco::Crypto::RSAKey(Poco::Crypto::RSAKey::KL_1024, Poco::Crypto::RSAKey::EXP_SMALL));
...
//    controllo se non ho gia invitato l'user2
    sql::Statement *stmt = connect();
    res=stmt->executeQuery("SELECT COUNT(`ID`) FROM `FILES_OWNERS` WHERE `ID`='"+user2->id+"' AND `FILENAME`='"+filename+"';");
    res->next();
    if(res->getInt(1)>0)
        return -1;
    ...
    mex=sender.SMTP_message_builder("New file shared: "+filename,"","Sender: "+user1->name+" "+user1->surname,"Open now!","http://www.facebook.it");
